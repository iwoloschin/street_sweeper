...
def parse_args():
    ...
    parser.add_argument(
        "-w",
        "--week",
        help="1st & 3rd (Odd) or 2nd & 4th (Even) Weeks?",
        nargs="+",
        choices=["odd", "even"],
        default=["odd", "even"],
    )
    parser.add_argument(
        "-d",
        "--day",
        help="Day of street sweeping",
        nargs="+",
        choices=calendar.day_abbr[0:5],
        default=calendar.day_abbr[0:5],
    )
    parser.add_argument(
        "-s", "--start", default=8, choices=range(24), help="Street sweeping start time (24 hr)"
    )
    parser.add_argument(
        "-e", "--end", default=12, choices=range(24), help="Street sweeping end time (24 hr)"
    )
    parser.add_argument(
        "-f",
        "--first",
